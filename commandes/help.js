...
module.exports.run = (client, message, args) => {
	...
	.then(message => {
        	...
        	message.edit(`:outbox_tray: |  Je vous envoie ça tout de suite : **...**`)
        	message.edit(`:outbox_tray: |  Je vous envoie ça tout de suite : **....**`)
        	message.edit(`:white_check_mark: |  Je vous ai envoyé les commandes en privé ! Si ceux-ci ne sont pas bloqués bien sûr.`).then(msg => msg.delete(5000))
        }) ;
    ...
    setTimeout(function() {
        ...
        .catch(err => {
            message.channel.send(`:x: |  Vous messages privée sont bloqué ! Veuillez les activer.`).then(r => r.delete(5000));
            })
    }, 1000);
