module.exports.run = (client, message, args) => {
    ...
    if (!member.roles.has(role.id)) { return message.channel.send(':x: |  Vous n\'avez pas ce role !').then(msg => msg.delete(5000)); }
    
        member.removeRole(role.id)
            .then(() => message.channel.send(':white_check_mark: |  Vous n\'avez désormais plus le role ' + role.toString())).then(msg => msg.delete(5000))
            .catch(console.error);
};
